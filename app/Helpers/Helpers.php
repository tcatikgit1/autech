...
class Helpers
{
  ...
  public static function newLog($method, $request=null, $exception=null, $msgCustom=null): void
  {
      $error = "Method " . $method;

      if(isset($request)){
          $error .= ' | Request: ' . json_encode($request->all());
      }

      if(isset($exception)){
          $error .= ' | Error: '. substr($exception, 0, 1000) . ' [Trimmed result]';
      }

      if(isset($msgCustom)){
          $error .= ' | Message: '. $msgCustom;
      }

      Log::channel('employee')->error($error);
  }

  // Función para formatear el presupuesto con dos decimales
  public static function formatear_presupuesto($presupuesto) {
    return number_format((float) $presupuesto, 2, '.', '');
  }

  public static function getId($id)
  {
    // Si el _id viene como un array con $oid, lo obtenemos de ahí
    if (is_array($id) && isset($id['$oid'])) {
      return $id['$oid'];
    }
    
    // Si el _id es directamente un string, lo retornamos tal cual
    if (is_string($id)) {
      return $id;
    }
