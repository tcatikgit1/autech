...
class AutonomoController extends Controller
{
    ...
    {
        $this->nameCrud = 'Autonomo';
    }
    public function index(Request $request){

        return view('content/boards/autonomos/board');
    }

    public function getUnvalidateAutonomo(Request $request){
        $url = "/api/autonomos/getUnvalidateAutonomo";
        $autonomo = APICallsController::callApi('get', config('app.GATEWAY_URL') . $url, $request->all(), $request);

        if($autonomo->getStatusCode() != 200){
            abort(404);
        }

        return $autonomo->getOriginalContent();
    }
...
//    }
    ...
    {
        return
            Route::controller(AutonomoController::class)->prefix('autonomos')->as('autonomo.')->group(function(){
                Route::get('/board', 'index')->name('index');
                Route::get('/getUnvalidateAutonomo', 'getUnvalidateAutonomo');
