...
>
    ...
    @php
        ...
        // Definimos el autor y cliente
        $autor = $anuncio['autor'] ?? $anuncio['autor'] ?? null;
        $cliente = $anuncio['cliente'][0] ?? $anuncio['cliente'][0] ?? null;
    @endphp
    ...
    <div class="card-body bg-white w-100 d-flex flex-column align-items-start rounded-bottom border-bottom border-1">
        ...
        <div class="d-flex justify-content-between w-100 align-items-center mt-2">
            ...
            {{-- Botones de acción --}}
            @if(is_null(session()->get('user')) || (isset($cliente['email']) && session()->get('user')['email'] != $cliente['email'])) {{-- Si el usuario está logueado y es el mismo que creó el anuncio, no se muestran los botones de chat y teléfono --}}
                <div class="d-flex">
                    @if(isset($autor) && !is_null($autor))
                        @if(isset($autor['is_data_visibility']))
                            @isset($cliente['telefono'])
                                <a href="tel:{{$cliente['telefono']}}" class="icon-circle me-3 no-redirect">
